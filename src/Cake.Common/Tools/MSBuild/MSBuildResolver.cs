...
namespace Cake.Common.Tools.MSBuild
{
    internal static class MSBuildResolver
    {
            ...
            MSBuildPlatform buildPlatform)
        {
            ...
            var visualStudio2022Path = environment.GetSpecialPath(SpecialPath.ProgramFilesX86);

            foreach (var edition in vsEditions)
            {
                // Get the bin path.
                var binPath = visualStudio2022Path.Combine(string.Concat("Microsoft Visual Studio/2022/", edition, "/MSBuild/Current/Bin")); // Change from Current to 16.0 after stable version released
                if (fileSystem.Exist(binPath))
                {
                    if (buildPlatform == MSBuildPlatform.Automatic)
                    {
            ...
            return visualStudio2022Path.Combine("Microsoft Visual Studio/2022/Professional/MSBuild/17.0/Bin");
        }
